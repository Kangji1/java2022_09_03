...
//               isOk = bsv.register(new BoardVO(title, writer, content));
         ...
         case "pageList":
        	 try {
        		 ...
        		 log.info(""+list.get(5).getBno());
        		 log.info(list.get(5).getImage_file());
					...
					case "new_file":
						if(item.getSize()>0) { //새로운 파일이 등록 됨
							...
							try {
								item.write(uploadFilePath); //자바 객체를 디스크에 쓰기
								bvo.setImage_file(fileName);
								log.info("uploadFilePath img_file"+(bvo.getImage_file()));
								//썸네일 작업
...
//               isOk = bsv.update(bvo);
         ...
         case "remove":
            try {
               ...
               FileHandler fileHandler = new FileHandler();
               savePath = getServletContext().getRealPath("/_fileUpload"); //생략가능
               isOk = fileHandler.deleteFile(imageFileName, savePath);
               log.info("fileDelete : "+(isOk>0? "OK":"Fail"));
               isOk = bsv.remove(bno);
               log.info("allfileDelete : "+(isOk>0? "OK":"Fail"));
               destPage = "pageList";
            } catch (Exception e) {
               e.printStackTrace();
            }
